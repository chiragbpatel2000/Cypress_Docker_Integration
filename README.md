...
# Technnical UI automation tests  with Cypress.io

## Why Cypress?
- Visit [Cypress](https://docs.cypress.io)

## Tech Info:
- Cypress version [v12.3.0]
...
2. Then run command `npm install cypress ` which will install cypress package with its dependancies as dependancies
...
2. E2E folder - This folder contains steps and tests sub folder where page object style is in use to export steps (from Steps folder) into specs files (Tests folder)
...
7. Docker Integration : we need to create dockerfile where the base image is taken from https://github.com/cypress-io/cypress-docker-images.

More Details follow below : # cypress/included
...
```
9. We might face some configuration issue around docker that docker daemon is not running , for that we have to take some extra step and can resolved that issue.
link given belwo : 

https://stackoverflow.com/questions/55301756/docker-daemon-is-not-running
